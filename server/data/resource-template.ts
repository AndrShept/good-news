...
const names = resourceTemplate.map((i) => i.name);
type ResourceTemplateName = (typeof names)[number];

export const resourceTemplateMap = resourceTemplate.reduce(
  (acc, template) => {
    acc[template.name] = template;
    return acc;
  },
  {} as Record<ResourceTemplateName, ItemTemplate>,
);
