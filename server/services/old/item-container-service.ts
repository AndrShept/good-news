...
export const itemContainerService = (db: TTransaction | TDataBase) => ({
    ...
    getBackpackId(heroId: string) {
    const hero = serverState.getHeroState(heroId);
    const backpack = hero.itemContainers.find((c) => c.type === 'BACKPACK');
    if (!backpack?.id) {
      throw new HTTPException(404, { message: 'backpack state not found' });
    }
    return backpack.id;
  },
  ...
  async consumeResources(itemContainerId: string, requiredResources: CraftItemRequiredResources[], containerSlots: any) {
    // for (const requiredResource of requiredResources) {
